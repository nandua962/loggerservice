...
func TestWavDecoder_Duration(t *testing.T) {

	t.Run("local file", func(t *testing.T) {
		config := audio.DecoderConfig{
			FilePath:     "samples/audio.wav",
			DecimalLimit: "3",
		}
	...
	// 	config := audio.DecoderConfig{
	// 		FilePath:     "https://www.learningcontainer.com/wp-content/uploads/2020/02/Kalimba-online-audio-converter.com_-1.wav",
	// 		DecimalLimit: "3",
	// 		RemoteFile:   true,
	// 	}

	// 	decoder := audio.NewWavDecoder(config)
	...
	t.Run("with custom bytes array", func(t *testing.T) {
		...
		decoder := audio.NewWavDecoder(config)
		f, err := os.Open("samples/audio.wav")
		require.Nil(t, err)
		defer f.Close()
